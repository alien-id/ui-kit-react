...
export const PriceChart = ({
  ...
  const [hoveredPoint, setHoveredPoint] = useState<PriceChartDataPoint | null>(
    null,
  );
  ...
  const displayPoint =
    hoveredPoint ?? (data.length ? data[data.length - 1] : null);
  ...
  const periodLabel = hoveredPoint
    ? formatTimestamp(hoveredPoint.timestamp, activePeriod)
    ...
    <div className={cn('alien-price-chart', className)}>
      <div className="alien-price-chart-header">
        <div className="alien-price-chart-info">
          <span className="alien-price-chart-value">
        ...
        >
          ...
          <span className="alien-price-chart-period-label">{periodLabel}</span>
        </div>
      </div>
